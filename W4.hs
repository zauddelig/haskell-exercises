...
isums :: Int -> IO Int
isums n = helper 0 n
    where helper sum 0 = return sum
          helper sum n = do n <- readLn
                            let sum' = sum + n
                            print sum'
                            helper sum' (n-1)
                            -}
...
hFetchLines h chosen_lines = do
    fil <- hGetContents h
    let total_lines = lines fil
    return (helper 1 chosen_lines total_lines)
    where 
        helper _ [] _ = []
        helper _ _ [] = []
        helper actual_number (chosen:other_chosen) (actual:next_lines)
            | actual_number == chosen = 
                actual:helper (actual_number+1) (other_chosen) (next_lines)
            | otherwise = helper (actual_number+1) (chosen:other_chosen) (next_lines)
...
process xs = case break (==',') xs of (a,[])    -> [a]
                                      (a,',':b) -> a:process b
-- din't get that until I found that "c" and 'c' behaves differently
...
compareFiles a b = do
    f_1 <- readFile a
    f_2 <- readFile b
    mapM_ putStrLn $ returnDifferentLines (lines f_1) (lines f_2)

returnDifferentLines [] [] = []
returnDifferentLines [] _ = ["< first continues"]
returnDifferentLines _ [] = ["> second continues"]
returnDifferentLines (l_1:lines_1) (l_2:lines_2) =
    if compareLines l_1 l_2 
       then returnDifferentLines lines_1 lines_2
       else ('<':' ':l_1):('>':' ':l_2):returnDifferentLines lines_1 lines_2

compareLines :: [Char] -> [Char] -> Bool
compareLines "" "" = True
compareLines _ "" = False
compareLines "" _ = False
compareLines (l1:line_1) (l2:line_2)
    | l1 == l2 = compareLines line_1 line_2
    | otherwise = False
...
interact' f state = do
    command <- getLine
    let (continue, toPrint, new_state) = f (command, state)
    putStr toPrint
    if continue then do
